...
public class PlayerDataJSON : MonoBehaviour
{
    private static string fileName = "PlayerDataFile";       //파일 이름
    PlayerDataContainer dataContainer;  //상품 데이터 변수
    ...
    public void LoadTopBarData()
    {
        ...
        {
            this.dataContainer = new PlayerDataContainer();  //객체 생성
            string saveJson = JsonUtility.ToJson(dataContainer, true);

            File.WriteAllText(savePath, saveJson);
        }
        else    //파일이 존재한다면
        {
            this.dataContainer = DataLoadText<PlayerDataContainer>(); //파일 로드
        }
    }

    public PlayerDataContainer GetTopBarData()
    {
        //데이터를 반환하는 함수
