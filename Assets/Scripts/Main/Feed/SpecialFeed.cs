...
public class SpecialFeed : MonoBehaviour
{
    ...
    public void LeftButton()
    {
        ...
        if (selectCount == 0)   //0개에서 감소 버튼을 누르면
        {
            ...
            float leftTime = this.gameObject.GetComponent<FeedTimer>().GetLeftTime(currentRackIndex);    //먹이 남은 시간
    ...
    public void selectSpecialFeed()
    {
        ...
        if (feedCount >= selectCount)
        {
            ...
            GetComponent<FeedTimer>().DecreaseFeedingTime(currentRackIndex, decrease);     // 선택한 횃대에만 특제 먹이 사용(남은 시간 감소)
