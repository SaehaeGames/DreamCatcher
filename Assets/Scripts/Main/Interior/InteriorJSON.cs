...
public class InteriorJSON : MonoBehaviour
{
    private static string fileName = "InteriorDataFile";       //파일 이름
    InteriorContainer interiorContainer;  //상품 데이터 변수
    ...
    public void LoadInteriorData()
    {
        ...
        {
            this.interiorContainer = new InteriorContainer();  //객체 생성
        }
        else    //파일이 존재한다면
        {
            this.interiorContainer = DataLoadText<InteriorContainer>(); //파일 로드
        }
    }

    public InteriorContainer GetInteriorData()
    {
        //데이터를 반환하는 함수
